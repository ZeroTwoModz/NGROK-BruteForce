...
namespace NGROK_BruteForce
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("Failed setting console mode!");
                Console.ReadKey();
            }
            else
            {
                ...
                {
                    Console.WriteLine(string.Format("Failed setting console mode!\n error code: {0}", (object)Program.GetLastError()));
                    Console.ReadKey();
                }
            }
            int good = 0;
...
░                                                                            ";
            void clear()
            {
                Console.Title = "NGROK BruteForcer";
                Console.Clear();
                WriteCenter(banner);
            ...
            clear();
            Console.Write(feri.Red + " Threads: " + feri.White);
            threadNumber = int.Parse(Console.ReadLine());
            Console.Write(feri.Red + " Timeout" + feri.Red + ": " + feri.White);
            timeout = int.Parse(Console.ReadLine());
            Console.Write(feri.Red + " Minimum Players" + feri.Red + ": " + feri.White);
            ...
            void updateTitle()
            {
                while (true)
                {
                    ...
                    cpm_aux = 0;
                    Console.Title = $"\r\r\rGood: {string.Format("{0:n0}", good)}       Bad: {string.Format("{0:n0}", bad)}       CPM: {string.Format("{0:n0}", cpm)}";
                    Thread.Sleep(500);
                }
            }
            StreamWriter streamWriter = new StreamWriter("working_servers.txt");
            ...
            void check(string host, ushort port)
            {
                try
                {
                    ...
                    if (ms.ServerUp)
                    {
                        ...
                        {
                            Console.WriteLine($" {feri.Grey}\n\n\r\n {feri.Red}Host: {feri.White + ms.Address}:{ms.Port}\r\n {feri.Red}Version: {feri.White + ms.Version}\r\n {feri.Red}Players: {feri.White + ms.CurrentPlayers}/{ms.MaximumPlayers}\r\n {feri.Red}Ping: {feri.White + ms.Latency}ms\r\n {feri.Red}MOTD: {color(ms.Motd)}");
